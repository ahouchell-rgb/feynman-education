...
| `20260621_01_topics_objective_id_and_subject_rpcs.sql` | **NOT APPLIED** | Multi-subject + mastery-graph wiring. `topics.objective_id` (FK guarded on `objectives` existing); guarded mirror into `topic_objective_map`; the weakness RPCs return `subject_id` + take optional `p_subject` (filters on `subjects.name`). `class_weak_topics` rebuilt from its live (Phase-3) gate, backward-compatible; `student_weak_topics` + `class_intervention_list` added net-new. `topics.subject_id` was already present/indexed on the anchor, so §1 is idempotent replay-safety only. |
