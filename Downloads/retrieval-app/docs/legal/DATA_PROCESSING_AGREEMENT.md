...
> `[BRACKETED]` items must be completed.

This Agreement is between **`[COMPANY]`** ("Processor") and the **school/customer**
("Controller") and forms part of the service contract. It reflects UK GDPR
Art. 28.
...
(Supabase, Anthropic, Vercel, and Microsoft for ScienceKit). The Processor will
inform the Controller of intended changes and impose equivalent obligations on
each sub-processor.

## 8. Return / deletion
On termination, the Processor will delete the Controller's personal data within
**`[N]` days**, and delete or anonymise pupil records, unless retention is
required by law. Per-pupil and per-class deletion is available on request during
the contract.

## 9. International transfers
Any transfer outside the UK/EEA is covered by `[UK IDTA / SCCs / adequacy]`.
...
- Backups and a documented restore process (see ops runbook). `[confirm plan/PITR]`
