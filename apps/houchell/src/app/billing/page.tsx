  ...
  return (
    <div>
      ...
      <div style={{ marginTop: 36, paddingTop: 18, borderTop: `1px solid ${C.rule}`, display: "flex", gap: 16, flexWrap: "wrap", fontFamily: C.mono, fontSize: 11.5, color: C.dim }}>
        <a href={`/api/account/export`} onClick={(e) => { e.preventDefault(); fetch("/api/account/export", { headers: { authorization: `Bearer ${sk.auth.getToken()}` } }).then((r) => r.blob()).then((b) => { const u = URL.createObjectURL(b); const a = document.createElement("a"); a.href = u; a.download = "houchell-data-export.json"; a.click(); URL.revokeObjectURL(u); }); }} style={{ color: C.muted, textDecoration: "none", cursor: "pointer" }}>⬇ Export my data</a>
