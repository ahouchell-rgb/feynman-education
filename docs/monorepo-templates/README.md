...
During Phase B (after `git mv` of the app into `apps/houchell/`), promote them to the repo root:

```bash
git mv apps/houchell/package.json apps/houchell/package.json   # app keeps its own manifest
...
turbo 2.x, three workspace members (`apps/houchell`, `apps/retrieval`, `packages/*`). `apps/interactive`
