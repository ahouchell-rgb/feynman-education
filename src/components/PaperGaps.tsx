...
/**
 * PaperGaps — the EXAM-data sibling of UnitGaps. For the class being taught, pulls
 * its weakest topics by marks lost on PAST PAPERS (ret.paperGaps → class_paper_gaps)
 * and offers a one-click "exam feedforward" sheet built from exactly those topics
 * (same /api/feedforward route, source:"exam" — exam-technique + mark schemes).
 *
 * Renders nothing unless the lesson has a linked retrieval class with enough marked
 * exam data — so it stays out of the way until there's something actionable.
 */
export function PaperGaps({ lessonId, contextClass, onSaved }: Props) {
  ...
  if (gaps && gaps.length === 0) return null;  // no exam data weak enough to show

  const genExamFeedforward = async () => {
    if (!gaps) return;
    setBusy(true); setErr("");
    try {
      ...
      const r = await fetch("/api/feedforward", {
        ...
        body: JSON.stringify({
          lessonId, className: contextClass?.name, source: "exam",
  ...
  return (
    <Card style={{ padding: 16, marginBottom: 24, borderLeft: `3px solid ${C.amb}` }}>
      <div style={{ display: "flex", alignItems: "baseline", justifyContent: "space-between", gap: 12, marginBottom: 10 }}>
        <div style={{ fontFamily: C.mono, fontSize: 11, fontWeight: 600, letterSpacing: "0.06em", textTransform: "uppercase", color: C.muted }}>
          Exam paper gaps{contextClass?.name ? ` · ${contextClass.name}` : ""}
        </div>
        <span style={{ fontFamily: C.mono, fontSize: 10, color: C.dim }}>past papers · marks lost</span>
      </div>

      {gaps === null ? (
        <div style={{ fontFamily: C.mono, fontSize: 12, color: C.dim }}>Loading exam data…</div>
      ) : (
        <>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 12, flexWrap: "wrap" }}>
            <Btn onClick={genExamFeedforward} disabled={busy}>{busy ? "Generating…" : "Generate exam feedforward"}</Btn>
            <span style={{ fontFamily: C.mono, fontSize: 10, color: C.dim }}>Exam-technique practice from marks lost · opens to print</span>
          </div>
