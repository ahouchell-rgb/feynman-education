...
export function SlideEditor({ deck, onChange, onUploadImage, onThemeChange, onMasterChange, onCurChange, autoQuestions }) {
  ...
  useEffect(() => {
    const onPaste = async (e) => {
      ...
      e.preventDefault();
      try {
        await placeImageFile(file, `slides/${deck.id}/${Math.floor(performance.now())}-paste`);
      } catch (err) { alert("Image paste failed — nothing was added to the slide: " + err.message); }
    };
    window.addEventListener("paste", onPaste);
    return () => window.removeEventListener("paste", onPaste);
  }); // re-bound each render so it closes over current slide / editing

  // Upload a file then place it as an image element — but only AFTER the upload
  // succeeds, so a failed upload never leaves a broken-src element on the slide.
  // The probe just measures the image for a sensible starting size; whether it
  // loads or not, the element gets the real uploaded URL (never a broken optimistic one).
  const placeImageFile = async (file, fallbackPath) => {
    const url = onUploadImage ? await onUploadImage(file) : await sk.upload(fallbackPath, file);
    await new Promise<void>((resolve) => {
      const probe = new Image();
      probe.onload = () => { const w = 420, h = Math.round(w * (probe.naturalHeight / probe.naturalWidth || 0.66)); addEl({ type: "image", x: 140, y: 120, width: w, height: h, src: url }); resolve(); };
      probe.onerror = () => { addEl({ type: "image", x: 140, y: 120, width: 420, height: 280, src: url }); resolve(); };
      probe.src = url;
    });
  };
  ...
  const pickImage = async (e) => {
    ...
    if (!file) return;
    try {
      await placeImageFile(file, `slides/${deck.id}/${Math.floor(performance.now())}-${file.name.replace(/[^\w.\-]/g, "_")}`);
    } catch (err) { alert("Image upload failed — nothing was added to the slide: " + err.message); }
  };
